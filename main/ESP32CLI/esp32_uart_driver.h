...
#define cmdANSI_ESC_RIGHT_FMT        ( (const char *) "\033[%udC" ) /* To be used with sprintf */
#define cmdANSI_ESC_LEFT_FMT         ( (const char *) "\033[%udD" ) /* To be used with sprintf */
...
#define cmdMOVE_CURSOR_RIGHT_POS(PORT, POS) {    \
    char tmpEsc[8] = {0, 0, 0, 0, 0, 0, 0, 0};   \
    sprintf(tmpEsc, cmdANSI_ESC_RIGHT_FMT, POS); \
    cmdSEND_ESC_CMD(PORT, tmpEsc);               \
}
#define cmdMOVE_CURSOR_LEFT_POS(PORT, POS)  {   \
    char tmpEsc[8] = {0, 0, 0, 0, 0, 0, 0, 0};  \
    sprintf(tmpEsc, cmdANSI_ESC_LEFT_FMT, POS); \
    cmdSEND_ESC_CMD(PORT, tmpEsc);              \
}
#define cmdMOVE_CURSOR_RIGHT(PORT) {          \
    cmdSEND_ESC_CMD(PORT, cmdANSI_ESC_RIGHT); \
}
#define cmdMOVE_CURSOR_LEFT(PORT)  {         \
    cmdSEND_ESC_CMD(PORT, cmdANSI_ESC_LEFT); \
}
